...
2026-09-23修复频繁锁定：解锁先获取新状态；心跳到期即排队，在上一请求完成后发送最新按键，仍只有一个在途行驶请求。新行驶指令用最近收到的桥时标加本地经过时间，避免不断重复旧时标；已发送指令不刷新时标，状态样本超过250ms仍停止。服务器开启TCP_NODELAY，雷达网络写出不再占控制锁，并保留每轮控制的首次锁定原因。300ms心跳、250ms旧指令及传感器保护不变。更新后刷新页面，不能承诺无线网络长延迟时继续控制。
