...
const client=crypto.randomUUID(), keys=new Set();
...
async function api(path,data){const ctrl=new AbortController(),timer=setTimeout(()=>ctrl.abort(),1000);try{const res=await fetch(path,{method:data?'POST':'GET',headers:{'X-Control-Token':token,...(data?{'Content-Type':'application/json'}:{})},body:data?JSON.stringify(data):undefined,signal:ctrl.signal,cache:'no-store'});const body=await res.json();if(!res.ok)throw Error(res.status===403?'访问令牌失效，请重新打开服务入口':body.error||res.status);return body;}finally{clearTimeout(timer);}}
function paintKeys(){document.querySelectorAll('[data-key]').forEach(b=>b.classList.toggle('pressed',keys.has(b.dataset.key)));}
function stop(reason='已停止并锁定'){active=false;generation++;keys.clear();paintKeys();notice(reason);api('/api/control',{op:'stop'}).catch(()=>{});}
async function send(force=false){if(!active||(pending&&!force))return;if(document.hidden||!document.hasFocus()||performance.now()-pollAt>250){stop('页面或连接失去活性，已请求停止');return;}pending=true;const own=generation;try{await api('/api/control',{op:'drive',client,boot,seq:++seq,tick,keys:[...keys]});}catch(e){if(own===generation)stop(e.message);}finally{pending=false;}}
$('stop').onclick=()=>stop();
$('arm').onclick=async()=>{if(active){stop();return;}keys.clear();paintKeys();try{const ack=await api('/api/control',{op:'arm',client,boot,seq:++seq,tick});armSeq=ack.bridge_seq;active=true;generation++;notice('控制已解锁 · 按住方向键输出，松开回中');send();}catch(e){notice(e.message);}};
...
window.addEventListener('blur',()=>{physicalKeys.clear();stop('窗口失焦，已请求停止并锁定');});
document.addEventListener('visibilitychange',()=>{if(document.hidden){physicalKeys.clear();stop('页面隐藏，已请求停止并锁定');}});
window.addEventListener('pagehide',()=>{active=false;fetch('/api/control',{method:'POST',headers:{'X-Control-Token':token,'Content-Type':'application/json'},body:JSON.stringify({op:'stop'}),keepalive:true}).catch(()=>{});});
document.querySelectorAll('[data-key]').forEach(b=>{b.onpointerdown=e=>{e.preventDefault();if(!active)return;if(b.dataset.key==='down'&&!state?.reverse_enabled)return;b.setPointerCapture(e.pointerId);keys.add(b.dataset.key);paintKeys();send();};const release=()=>{keys.delete(b.dataset.key);paintKeys();send(true);};b.onpointerup=release;b.onpointercancel=()=>stop('触控中断');b.onlostpointercapture=release;});
...
const reasons={heartbeat_timeout:'心跳超时',control_gap:'控制循环中断',stale_request:'旧指令已拒绝',operator_stop:'已停止',locked:'已锁定',not_armed_or_invalid_request:'需要重新解锁',status_backpressure:'状态通道阻塞'};
async function poll(){try{const s=await api('/api/state');state=s;boot=s.boot;tick=s.status.control?.tick||0;pollAt=performance.now();const c=s.status.control||{};if(active&&!c.armed&&c.seq>=armSeq){active=false;keys.clear();paintKeys();notice(reasons[c.reason]||c.reason||'控制已锁定');}
if(s.healthy&&$('notice').textContent==='等待画面与雷达就绪，默认锁定。')notice('实时画面已连接，控制保持锁定。');$('connection').textContent=s.healthy?'● 传感器在线':'● 检查连接';$('connection').classList.toggle('good',s.healthy);$('armed').textContent=c.armed?'已解锁':'已锁定';$('arm').textContent=active?'锁定控制':'解锁键盘控制';$('arm').disabled=!s.healthy||(!active&&s.owner&&s.owner!==client);$('motor').innerHTML=(c.motor||1500)+'<span>µs</span>';$('servo').textContent=c.servo||1500;$('camera-age').textContent=s.ages.camera<5?Math.round(s.ages.camera*1000)+' ms':'无新画面';$('mode').textContent=s.demo?'模拟模式 · 无实车输出':'实时画面';$('reverse-note').textContent=s.reverse_enabled?'倒车输出已启用；具体制动/倒车行为取决于电调。':'倒车待标定，当前锁定。↓不会自动执行电调倒车序列。';document.querySelector('[data-key="down"]').disabled=!s.reverse_enabled;
