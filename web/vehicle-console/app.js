...
$('arm').onclick=async()=>{if(arming)return;if(active){stop();return;}keys.clear();physicalKeys.clear();paintKeys();arming=true;const attempt=++generation;notice('正在解锁…');try{const fresh=await api('/api/state');if(attempt!==generation)return;if(!fresh.healthy)throw Error('相机、雷达或控制连接未就绪');if(fresh.owner)throw Error('已有页面占用控制，请先按停止并锁定，再解锁');boot=fresh.boot;tick=fresh.status.control.tick;pollAt=performance.now();const ack=await api('/api/control',{op:'arm',client,boot,seq:++seq,tick});if(attempt!==generation){stop('解锁已取消，请重新点击');return;}armSeq=ack.bridge_seq;active=true;generation++;notice('控制已解锁 · 按住方向键输出，松开回中');send();}catch(e){notice(e.message);}finally{arming=false;}};
...
const reasons={'stale/unowned request; unlock again':'控制会话已结束，请重新解锁','stale arm request':'解锁请求已过期，请重试','already armed; stop first':'控制已被占用，请先停止并锁定',stale_or_unowned_request:'控制会话已结束，迟到指令被拒绝',bridge_not_armed_or_invalid_request:'底盘未解锁或指令无效',invalid_keys:'无效方向指令',browser_timeout:'浏览器心跳超时',sensor_stale:'传感器数据过期',bridge_heartbeat_timeout:'底盘心跳超时',bridge_control_gap:'底盘控制循环超时',bridge_stale_request:'控制请求延迟或乱序',bridge_status_backpressure:'底盘状态通道阻塞',heartbeat_timeout:'心跳超时',control_gap:'控制循环中断',stale_request:'旧指令已拒绝',operator_stop:'已停止',locked:'已锁定',not_armed_or_invalid_request:'需要重新解锁',status_backpressure:'状态通道阻塞'};
