...
$('stop').onclick=()=>stop();
$('access-form').onsubmit=e=>{e.preventDefault();const value=$('access-token').value.trim();if(!/^[A-Za-z0-9_-]{43}$/.test(value)){notice('请输入43位访问码');return;}location.hash='token='+value;location.reload();};
...
async function poll(){try{const s=await api('/api/state');state=s;boot=s.boot;tick=s.status.control?.tick||0;pollAt=performance.now();const c=s.status.control||{};if(active&&!c.armed&&c.seq>=armSeq){stop((reasons[c.reason]||c.reason||'控制已锁定')+'；请重新解锁');}
if(s.healthy&&$('notice').textContent==='等待画面与雷达就绪，默认锁定。')notice('实时画面已连接，控制保持锁定。');$('connection').textContent=s.healthy?'● 传感器在线':'● 检查连接';$('connection').classList.toggle('good',s.healthy);$('armed').textContent=c.armed?'已解锁':'已锁定';$('arm').textContent=active?'锁定控制':'解锁键盘控制';$('arm').disabled=false;$('arm').title=!s.healthy?'传感器或控制连接未就绪':s.owner&&s.owner!==client?'其他页面占用控制，可先停止再解锁':'';$('motor').innerHTML=(c.motor||1500)+'<span>µs</span>';$('servo').textContent=c.servo||1500;$('camera-age').textContent=s.ages.camera<5?Math.round(s.ages.camera*1000)+' ms':'无新画面';$('mode').textContent=s.demo?'模拟模式 · 无实车输出':'实时画面';$('reverse-note').textContent=s.reverse_enabled?'倒车输出已启用；具体制动/倒车行为取决于电调。':'倒车待标定，当前锁定。↓不会自动执行电调倒车序列。';document.querySelector('[data-key="down"]').disabled=!s.reverse_enabled;
