...
| 蓝牙 | 验证未通过：无HCI控制器，限时扫描提示No default controller available；厂商初始化所需rtk_hciattach缺失，尚未修复，不能据此判硬件损坏 |
...
蓝牙修复已准备配套工具和固件，按用户要求先临时扫描验证、通过才装系统；当前等待车端sudo执行。[修复步骤与状态](docs/车端蓝牙修复.md)。
