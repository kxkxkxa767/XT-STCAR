...
| USB相机 | `/dev/video20`已成功读取5帧640×480图像，真实图像已送入Rust原生YOLO推理 |
| 雷达 / IMU | 3秒采集：雷达844包/13,504点/30次角度回绕；IMU299样本，平均加速度模长9.819m/s²；两者校验错误均0。尚未标定安装外参和零偏 |
| ONNX Runtime | 系统1.18.1不支持API22，真实加载失败；独立测试目录的官方SpacemiT 2.0.6包（ORT1.24.2）已实际推理成功，未替换系统库 |
| 规划与任务 | 完整固定场目标端执行通过：模拟75.6秒/757拍，主机耗时105.64秒；首次90秒超时保留。仅合成闭环，在线正常整场此前0/15完成 |
| 底盘 | 用户确认1450向右、1550向左、1500回中；架空0.5秒油门测试中1510/1519未起转，1520向前转且回1500后停住。1515脉冲期间是否转动未确认。独立守护补停检查通过；速度/转角/制动时间未标定 |
...
实际命令、结果和限制见 [车辆到场模块检查](docs/车辆到场模块检查.md)及[机器验证记录](docs/vehicle-bringup-validation.json)。
