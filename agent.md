...
用户明确要求把后续视觉识别工作写入本文件，交由队友完成。以下是待办与验收要求，不表示已经实现。训练由队友负责；本次助手只更新文档，不改代码、不训练、不部署、不操作车辆。用户曾请求自动前进到斑马线前停车，但本轮未执行任何运动；之后用户手动到白线处，又明确说已回发车区。**最新位置以用户“已回发车区”为准，尚未在该次回位后重新采样，不能把之前线前照片当当前画面。** 后续接手重新读取状态；本轮已有运动请求不作为队友任意试车的持续授权，电机动作仍需用户明确指定当次范围。
...
- 给白色mask、连通域和最终分组增加可显式启用的有界诊断：候选数量、像素轮廓、面积、填充率、长短轴、方向、间距，以及首个拒绝原因。区分ROI/灯区域排除、白条与反光地面连通、候选框覆盖、面积/长宽比、组内方向/重叠/宽长比/间距、条数、投影/物理尺寸、边界裁切、最终元素确认等原因。
- `stripe_axis=either` 已允许任意主轴，不能把“加一个允许旋转开关”当作修复。现算法的像素方向/长宽比/间距假设可能受透视影响，但本轮尚未做逐阶段复现，具体漏检原因未证实。
- `proposal_covers_blob` 在模型未声明Crosswalk角色时允许经典RGB分支继续工作；**单蓝桶模型不等于完全没有白条检测功能**。新模型声明Crosswalk角色后，现行为会要求模型候选覆盖白条，需测试漏框/截断框的行为。若增加无框后备，应显式设计其可信度和准入条件，不能悄悄绕过候选约束。
...
- 评估局部亮度对比/边缘与颜色联合分割，处理灰白反光地面、曝光变化、阴影、白墙和蓝挡板。只下调白色阈值容易把地板连成一片，需用真实负例验证。
- 从白条四边形/边缘建立条纹组合，允许透视下逐渐变化的条宽、间距与长度，使用共同投影关系和排列一致性，而非要求图像里近似等宽等距。局部图案透视归一化可辅助识别；没有尺度/标定依据时不能据此生成绝对距离。
...
- 保留暗灯、过曝、同帧红绿冲突、模型颜色与RGB冲突、低置信/过期时Unknown或停车语义；明确冲突规则并测试。绿灯标签不提供灯前停车区几何，也不能直接授权通行；原独立帧确认与绿灯等待门不放宽。
...
- 优先测能独立运行的轻量白条几何分支，避免白线提示被单蓝桶推理串行拖慢；若拆分，仍沿用同一相机源帧/时间和独立有效期，并验证不会挤占原心跳/传感器读取。性能是否够用由停车反应预算决定，不能只报平均FPS。

#### 追加：转弯后锥桶出画、雷达继续跟踪与纯雷达候选（2026-09-23）

用户新增需求：转弯后不一定能看到锥头/锥桶，但雷达仍可能看到两桶，希望仍按比赛要求绕行。此处记录需求与待修正设计，不代表当前已经有可用实车自主链路，不直接实施“任意两个点簇都算锥桶”的规则。

**现有代码已有能力，先接通并验证，不重复造一套：**

- `crates/robot-core/src/local_world.rs::maintain_confirmed_cones` 已允许已视觉确认、有效且未处理的锥桶出画后，通过同刻位姿/雷达和显式外参唯一关联，更新同一TrackId的几何。`last_visual_at` 和 `last_geometry_at` 分离，默认视觉语义20秒、几何1.8秒；关联歧义、不足质量、过期等均不能续接，雷达不伪造颜色、视觉确认次数或视觉时间。
- `crates/runner/src/online.rs` 已调用该接口，已有 `online_adapter_maintains_confirmed_cone_from_side_scan_after_camera_disappears` 回归。该测试不等于实车已打通。当前驾驶台vision-shadow仍是只读视觉诊断，不会因为雷达网页能显示点云而自动运行OnlineSession。
- 首先区分“只看不见尖顶，桶身/底脚仍可见”“整个目标转到相机视场外”“该目标从未视觉确认”“关联/定位丢失”四类，不把它们都解释成模型识别差。检查 `road.rs::detect_elements` 的最小高度/高宽比/锥度及 `element_blob_unclipped` 是否拒绝局部桶身；给拒绝原因诊断。能看见桶身但尖顶裁切时可研究局部语义候选；底脚不可见时不能拿裁切框底边作地面脚点。

**队友实施顺序与边界：**

1. 优先验证“先视觉确认身份→出画→雷达持续更新几何→继续同一桶的绕行”。任务锁定活动TrackId，身份不能随每帧最近点簇更换，已绕过目标不能因重检换ID再绕一次。区分确认不足、语义到期、几何过期、拟合歧义、遮挡和定位异常，并输出具体停因。
2. 核对雷达维护是否被视觉发布节奏拖慢：现有OnlineSession的输入约束要求同一采集时间的road/elements/pose与匹配扫描。若另建按雷达节奏维护的入口，应真实关联/插值位姿并保留各传感器原始时间、误差和独立新鲜度；不能复制旧视觉并改时间戳充作新帧，也不能用收到结果的时间冒充曝光时间。
3. 对从未视觉确认的目标，新增的是**无颜色、未确认的雷达几何候选**，而不是直接创建已确认Cone。候选需多帧稳定、足够有效点/覆盖、合适尺寸和拟合残差；排除长墙、挡板脚、墙角及碎片。扫描高度截到锥体还是方底座需实测，不能强行把所有截面按圆拟合。实现需要记录候选来源、独立确认状态与不确定度，不伪造VisualLidar来源。
4. 若希望纯雷达候选也可进入任务，需显式设计可选的“无颜色几何任务目标”准入，并核对实际赛场/规则能否仅靠布局、起点朝向与连续观测唯一确定第一/第二桶及绕行侧。当前 `OnlineMissionConfig.required_cone_colors` 默认None只表示不要求颜色，并不等于任何雷达点簇均可成为确认目标。存在两个相似障碍/身份或顺序歧义时不能任意选择；保持停止或采用有界、已证空闲的重新观察策略。
5. 赛事路径指任务顺序与绕行拓扑，不是固定油门时序或沿规则示意图像素轨迹走。依据规则图核对第一/第二桶、通过侧、实际半圈与出口关系；`online_mission.rs::cone_step/select_track` 需保持目标身份、已处理排除、真实绕行进度和完成门。短目标随实时锥桶几何更新，整车足迹/墙体/未知区/制动包络/最终采用证书仍生效。
6. 不通过删除视觉有效期、无限续期、跳过唯一关联或填模拟位姿来“修复”。若实测正常绕桶确实超过当前20秒语义有效期，作为新的身份确认策略单独设计和论证（重观察或带证据的纯雷达模式），保留原默认兼容；不先随意调TTL追求完成。
7. 补回归：转弯后全桶出画但扫描连续、尖顶裁切/底脚裁切、两桶同色、从未视觉确认的孤立簇、仅一桶/多余障碍、墙角/挡板端点、两簇合并分裂/遮挡重现、同一簇争抢两个ID、定位跳变、旧帧/重复帧、几何或语义到期、已处理桶重检，以及第一桶到第二桶的顺序和通过侧。连续雷达维护不得增加视觉确认数或刷新视觉时间。
8. 验收分层：静态真实数据聚类/身份证据→有真值标注的离线序列与模拟任务→车端只读实时跟踪→另行明确授权后的受限运动。纯雷达候选只有在身份/定位/控制全链路验证后才能用于执行；不能由本条需求直接恢复实车自动油门。
...
2. **功能回归**：重点覆盖 `crates/vision/tests/road.rs`、`crates/vision/tests/ground_markers.rs`、semantics/model契约测试、`tests/test_yolo26_tools.py`、`tests/test_vision_shadow.py`；涉及页面再测 `web/vehicle-console/tests/`。覆盖旋转/透视/尺度/曝光、反光与普通白线负例、少条/截断/过曝、空候选、重复/乱序/过期帧、错误类别顺序、红绿冲突、未标定/无效投影和资源上限。原官方80类与单蓝桶配置必须保持兼容。
...
6. **明确单列运动前置条件**：视觉通过后仍需可靠起步/低速、制动距离与延迟、车体足迹/外参、侧向碰撞检查、独立急停和最终控制门。未来移动必须有距离/时长上限、丢失或旧视觉停止、用户现场可急停；不能执行没有终止上限的“直到识别才停”，也不能由聊天轮询截图代替本机实时控制循环。当前不声称这些条件已满足，不凭本任务单启动电机。
