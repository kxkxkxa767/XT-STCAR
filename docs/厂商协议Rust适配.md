...
2026-09-08 又加入 [N10 Rust 解析](N10协议依据.md)、[物理单位标定预览](底盘标定映射.md)
以及独立的 [Rust 串口采集入口](Rust串口采集.md)；后者默认计划，显式 `--execute` 才打开传感器 tty。
...
日志保留解码样本、坏校验/丢弃字节/过期分量统计。同一读取块包含多个样本时，完整 `imu_decode`
只写在首条 step，后续 step 以 `imu_sample_index` 标识该块内的样本，避免重复记录整个批次。没有完整有效样本的块只作为 Tick 处理，不刷新传感器时间。
...
没有把厂商转向系数直接用于 MotionIntent；未移植带旧标定的 C++ 节点；现已依据 N10 源码增加独立解析，导航算法尚未移植，未读取视频。
